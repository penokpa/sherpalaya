    ...
    <div class="bg-blue-100/50">
        <div class="card--rounded-none image-full bg-blue-100/50 h-[80dvh] relative">
            <figure class="h-[80dvh] w-full">
                <x-curator-glider class="h-[80dvh] w-full object-cover brightness-75" :media="$pageSetting->peak_page_cover_image_id ?? null" :fallback="asset('/photos/banner.jpg')"
                    loading="lazy" />
            </figure>
            ...
            @foreach ($peaksRegion as $peakRegion)
                @if ($peakRegion->peaks->isNotEmpty())
                    <div id="region-{{ $peakRegion->id }}">
                        <h5 class="card-title mb-2.5 line-clamp-2 uppercase text-2xl text-primary text-center font-semibold"
                            data-aos="fade-down" data-aos-duration="1200">
