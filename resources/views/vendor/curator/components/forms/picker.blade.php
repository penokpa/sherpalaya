    ...
    <div
        x-data="{
            insertMedia: function (event) {
                ...
                // Defer one tick so the same-batch close-modal handler and
                // Filament's morphdom settle before we commit state. Without
                // this, the picker briefly shows the image then snaps back
                // to empty because the close-modal re-render races our $set.
                setTimeout(() => $wire.$set(path, media), 0);
            },
        }"
