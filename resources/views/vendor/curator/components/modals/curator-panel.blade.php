<div
    x-data="{
        ...
        forwardDropToFilePond: function (event) {
            ...
            const fpRoot = this.$el.querySelector('.filepond--root');
            if (! fpRoot) return;

            const alpineEl = fpRoot.closest('[x-data]');
            if (alpineEl && window.Alpine) {
                const data = Alpine.$data(alpineEl);
                if (data && data.pond && typeof data.pond.addFiles === 'function') {
                    data.pond.addFiles(files);
                    return;
                }
            }
        },
    ...
    class="curator-panel h-full absolute inset-0 flex flex-col relative"
    x-on:dragover="if (isFileDrag($event)) { $event.preventDefault(); isDragging = true; }"
    x-on:dragenter="if (isFileDrag($event)) { $event.preventDefault(); isDragging = true; }"
    x-on:dragleave="if (isDragging && ($event.relatedTarget === null || ! $el.contains($event.relatedTarget))) isDragging = false"
    x-on:drop="if (isFileDrag($event)) { $event.preventDefault(); forwardDropToFilePond($event); }"
>
    <!-- Toolbar -->
    ...
    {{-- sherpalaya: reminder banner so users don't forget to save the parent form --}}
    <div class="px-4 py-2 text-xs bg-amber-100 text-amber-900 dark:bg-amber-500/10 dark:text-amber-200 border-b border-amber-300/40 dark:border-amber-500/20 flex items-start gap-2">
        <x-filament::icon
            icon="heroicon-m-information-circle"
            class="w-4 h-4 mt-px shrink-0"
        />
        <span>
            <strong>Drop files anywhere in this dialog</strong> to upload, then click <strong>Add Files</strong> &rarr; <strong>Use Selected Image</strong>.
            After closing the dialog, click <strong>Save changes</strong> on the form — otherwise the image won&rsquo;t be linked.
        </span>
    </div>

    {{-- sherpalaya: drag overlay so users see where the drop target is --}}
    <div
        x-show="isDragging"
        x-cloak
        x-transition.opacity
        class="absolute inset-0 z-50 flex items-center justify-center pointer-events-none bg-primary-500/10 ring-4 ring-inset ring-primary-500/50"
    >
        <div class="px-6 py-4 rounded-lg bg-gray-900/90 text-white text-sm font-medium shadow-xl">
            Drop to upload
        </div>
    </div>
