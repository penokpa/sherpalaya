<div id="recommended" class="bg-blue-100/20">
    @if ($recommendedTreks->isNotEmpty())
        ...
        <h5 class="card-title text-secondary uppercase font-normal text-3xl">
            More {{ $trek->region->name }} Region Treks
        </h5>
        <div class="h-6"></div>

        <div class="lg:grid grid-cols-3 gap-2 flex flex-col">
            @foreach ($recommendedTreks as $recommendedTrek)
                <div class="card group hover:shadow sm:max-w-sm bg-blue-100/80 h-full overflow-hidden">
                    <a href="{{ route('show_trek', $recommendedTrek->id) }}">

                        <figure class="h-44">
                            <img loading="lazy" decoding="async" src="{{ $recommendedTrek->coverImage?->url }}"
                                alt="{{ $recommendedTrek->title }} Cover Image"
